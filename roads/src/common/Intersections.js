...
export function getLineLineIntersections( x1, y1, x2, y2, x3, y3, x4, y4 ) {
  ...
  if ( D != 0 ) {
    ...
    // If collinear lines overlap, where is the intersection?
    // Find closest pair of points and split difference?

    // Just handle simplest case of them barely touching for now, since this
    // could come up if lines are pieces of grid
    if ( x1 == x3 && y1 == y3 )   return [ [ x1, y1 ] ];
    if ( x1 == x4 && y1 == y4 )   return [ [ x1, y1 ] ];
    if ( x2 == x3 && y2 == y3 )   return [ [ x2, y2 ] ];
    if ( x2 == x4 && y2 == y4 )   return [ [ x2, y2 ] ];
  }
